...
class App(QMainWindow, QWidget):  # 창의 대부분의 기능
    ...
    def btn_list(self,name):
        self.btnList = []
        self.btnTop = 100
        FileListBox = QVBoxLayout()

        for i in range(len(name)):
            self.btnList.append(self.button_create(name[i]))
            self.btnList[i].resize(QSize(80, 25))
            self.btnList[i].move(10, self.btnTop + (i * 25))
            FileListBox.addWidget(self.btnList[i])
        return FileListBox
    ...
    def createMainView(self):
        ...
        hexa_info_box.addWidget(self.mainTextArea, 6)
        hexa_info_box.addWidget(self.asciiTextArea, 2)
        File_list_box.addLayout(self.btn_list(list_dir))
        totalBox.addLayout(File_list_box)
