...
class App(QMainWindow, QWidget):  # 창의 대부분의 기능
    ...
    def readFile(self, fileName):
        ...
        sector_number = 0
        self.generateView(self.read_FAT_DATA.read_sector(0), sector_number)

        #self.generateView(self.read_FAT_DATA.read_sector((6 - 2) * self.read_FAT_DATA.spc + 8192))
    ...
    def generateView(self, text,sector_number):
        ...
        file_button_list_info=self.btn_list(file_button_data)
    ...
    def file_generateView(self, text,sector_number):
        space = ' '
    ...
    def button_create(self, name):
        ...
        # button_list_info[i].layoutDirection(Qt.RightToLeft)
        ...
        for i in self.read_FAT_DATA.dir_list:
            if (name==i['sname']) or ('name' in i and name ==i['name']) :
                if 'del' in i:
                    ...
                    #self.read_FAT_DATA.renew_list()
                    self.generateView(
                        self.read_FAT_DATA.read_sector((self.read_cluster - 2) * self.read_FAT_DATA.spc + 8192),
                        (self.read_cluster - 2) * self.read_FAT_DATA.spc + 8192)# 8192 는 데이터 시작영역
        ...
        for i in self.read_FAT_DATA.file_list:
            if (name==i['sname']) or ('name' in i and name ==i['name']) :
                ...
                if 'size'==0 in i: #파일 목록에서 디렉토리일때
                    ...
                    self.generateView(
                        self.read_FAT_DATA.read_sector((self.read_cluster - 2) * self.read_FAT_DATA.spc + 8192),
                        (self.read_cluster - 2) * self.read_FAT_DATA.spc + 8192)
                else:
                    self.read_cluster=i['cluster']
                    if self.read_cluster == 0:
                      self.read_cluster=2
                   # self.read_FAT_DATA.renew_list()
                    self.file_generateView(
                        self.read_FAT_DATA.read_sector((self.read_cluster - 2) * self.read_FAT_DATA.spc + 8192),
                        (self.read_cluster - 2) * self.read_FAT_DATA.spc + 8192)  # 8192 는 데이터 시작영역
    ...
    def createMainView(self):
        ...
        #file_list_Widget.setAlignment(QtCore.Qt.Alignment)
        #file_list_Widget.setStyle(QColor(53, 53, 53))
